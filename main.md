...
프론트엔드 개발을 중심으로 지도 기반 서비스, 자동화 시스템, 성능 최적화 경험을 쌓아온 개발자입니다.

React 기반 UI 개발뿐 아니라 Puppeteer 기반 브라우저 자동화, WebGL 환경 문제 해결, Docker 및 인프라 구성까지 폭넓게 다루며 실제 서비스 환경의 문제를 해결해왔습니다.

불완전한 요구사항을 구조화하고, 성능 개선과 구조 설계를 바탕으로 UI와 기능 흐름을 빠르게 구현하는 데 강점이 있습니다.
...
- **Visualization**: Mapbox GL, MapLibre GL, Three.js, WebGL
...
- 대용량 지도 데이터 렌더링 구조 개선
- 사용자 인터랙션 성능 개선
- CI/CD 파이프라인 일원화를 통한 배포 자동화 및 운영 효율 개선
...
- React 기반 사용자 웹 서비스 개발
- Mapbox GL을 활용한 지도 UI 및 인터랙션 구현
...
- 빠른 화면 구현을 통해 기획 의도를 구체화하고 개발-기획 간 협업 효율 개선
...
- 운영 효율 개선 및 관리 기능 고도화

#### 지도 녹화 자동화 시스템

**주요 구현**

- Orchestrator API를 통해 녹화 요청을 관리하고, BullMQ 기반 작업 큐로 워커를 제어하는 구조 설계
- Puppeteer 기반 브라우저 자동화로 지도 재생 및 영상 녹화 처리 구현
...
- 수동 녹화 작업을 서버 기반 비동기 처리로 자동화
- BullMQ 기반 큐 구조를 통해 작업 처리 안정성 및 확장성 확보
- 병렬 처리 구조 도입으로 대량 영상 생성 처리 효율 개선
- 서버 환경에서 WebGL 기반 지도 렌더링 및 녹화 안정화
...
- React 환경에서 지도 기능을 재사용 가능한 구조로 추상화
- 반복적인 지도 연동 코드 감소
- npm 패키지 공개 배포 및 문서 기반 사용 환경 제공
...
- 영상 통화 기반 신고 기능 도입으로 현장 상황 전달 정확도 향상
- 관리자 시스템을 통해 실시간 대응 및 운영 효율 개선

#### 소방대원 실내 위치 추정 및 관제 시스템

**2025.04.01 ~ 현재**

화재 현장과 같은 실내 환경에서 소방대원의 위치를 추정하고, 이를 시각화하여 실시간으로 모니터링할 수 있는 관제 시스템입니다.
...
- 2D 지도와 3D 시각화를 결합한 모니터링 환경 구축
- 실시간 데이터 반영 시 성능 저하 없이 안정적인 렌더링 구조 구현
